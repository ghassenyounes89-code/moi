...
axios.defaults.baseURL = "https://bakendmoi-5.onrender.com"
...
  
  // Reverting to the original state structure
...
  // We direct the Login request to the correct backend endpoint: /api/visitor/login
  const handleLogin = async (e) => {
    e.preventDefault()
    setLoading(true)
    try {
      // The backend uses email and name for login/creation. Pass email, and optionally the user's name if they update it.
      const response = await axios.post("/api/visitor/login", { email: loginForm.email, name: loginForm.name }) 
      const { token, visitor: user } = response.data // Destructure 'visitor' from backend as 'user' for frontend consistency
...
      setCommentForm((prev) => ({ ...prev, name: user.name, email: user.email }))
      // Note: Setting loginForm back to original structure, although 'password' isn't used by the backend.
...
  // *** CORRECTION APPLIED HERE ***
  // We direct the Register request to the correct backend endpoint: /api/visitor/login
  const handleRegister = async (e) => {
    e.preventDefault()
    setLoading(true)
    try {
      // The backend creates a new visitor if the email doesn't exist. Pass name and email.
      const response = await axios.post("/api/visitor/login", { name: registerForm.name, email: registerForm.email }) 
      const { token, visitor: user } = response.data 
...
    e.preventDefault()
    ...
    // Payload respects user login status
    const payload = user ? { message: commentForm.message } : commentForm;

    if (!payload.message || (!user && (!payload.name || !payload.email))) {
...
                <a
                  href="https://www.tiktok.com/@younes.cs3?_t=ZS-90FdS5uY46j&_r=1"
                  target="_blank"
...
                </a>
                <a
                  href="https://www.instagram.com/gha_sen.0/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="social-link"
...
                {loading ? "Submitting..." : (user ? "Submit Comment" : "Login to Submit")}
              </button>
            </form>
          </div>
...
                className="form-input"
              />
              {/* Keeping Password field for frontend consistency, though backend ignores it */}
              <input
                type="password"
...
        <div className="container">
          <p>&copy; 2025 Ghassen Dev. All rights reserved.</p>
        </div>
      </footer>
    </div>
  )
